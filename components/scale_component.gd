...
func tween_scale() -> void:
	...
	tween.tween_property(target, "scale", scale_amount, scale_duration * 0.1).from_current()
	...
	tween.tween_property(target, "scale", Vector2.ONE, scale_duration * 0.9).from(scale_amount)
